...
export function buildDetailedOutlineActPrompt(input: {
  ...
  return [
    ...
    'summary、facts、sceneByScene 都不要解释“象征了什么”“说明了什么”“哪套大道被领悟”；主题只能藏在动作后果和代价里，不准翻成作者解说。',
    'summary、episode summary、sceneByScene 里禁止使用“争证据”“争站队”“争时间”“主导权”“推进”“升级”“收束”这类 writer-room 词；都要翻成谁抢盒、谁堵门、谁把哪件东西拍到谁脸上。',
    '前 1-6 集不要反复直说“谦卦”“不争”“大道”“真镇守”；先写这套东西怎样逼黎明忍、让、藏、换条件、护人或反咬。进入当前 5 集批次的收口段后才允许点题，而且每集最多 1 次，必须贴着实物或已发生的后果，不准讲经。',
    '第6集以后，每集 summary 第一短句必须先落在外场或私下动作：搜屋、拦路、医治、抢证、追残党、毁契、换手、押送路上、门外堵截至少一种；如果第一句还是堂上流程、关押问话或盖章程序，说明主戏眼写歪了。',
    ...
    ...rankIdentityRules,
    ...monsterFactRules,
    '凡是主题、领悟、旧规、空物、信条类正式事实，当前批次末两集至少要把词面或直接同义动作落出来；像“谦卦”“不争”“空无一物”“守心”不能只藏在总述。',
    '如果正式事实里确认主角有“隐忍/藏锋/先让后反咬”，前 1-6 集至少两集必须显性落出“黎明隐忍”“黎明藏锋”“黎明装弱不亮底”或“黎明先忍住不出手”之一；不准只写他突然会打。',
    '如果已确认正式事实里有“妖兽/蛇子/镇妖地/封印外压”，开局段和收束段都必须显性落一次；不要只把它写成早期背景或别人嘴里的旧事。',
    '收束段至少一集要直接写蛇子异动、鳞片外渗、镇妖地受扰或妖兽反噬怎样继续逼人站队、加重职责或抬高代价；不准把妖兽外压在后段完全写没。',
    '每一集先钉住一个本集戏眼：这一集到底在争人、争物、争证据、争名声、争时间还是争站队；summary 和 sceneByScene 都围着这个戏眼写。',
    '每一集都要写清：这一集最具体的推进动作、最直接的压强、最明显的代价变化、集尾继续点开的口子。',
    '“拿刀抵喉/绑住人逼交钥匙/抓住小柔逼黎明现身”这种直给压法全季最多 2 次；上集用过，本集就换成证据、旧规、伤势、父辈、名声、残党、账册或职责筹码，别连续复制。',
    ...
    '妖兽、灾变、高手外压只能放大人祸，不能替代人祸；哪怕有怪物，也要明确是谁借它做局、谁拿它逼人、谁因它失势。',
    '终局不能只剩大战收尾；必须把前面的人账、证据账、规则账或关系账至少收掉两条，让外压服务于收账，而不是替代收账。',
    '如果当前已经进入收束段，至少一集要直接写谁被揭穿、谁失去筹码、谁被迫站队、谁拿证据换命或谁被追责，不准把后三集的主推进连续写成长老议事、法阵压制或世界异象说明。',
    '师父、长老、高手在详纲里只能改规则、压时限、给条件、逼表态；不能直接执行“废修为、收钥匙、投入炼炉、当众宣判”这类终局动作。真正把人账落地、把关系翻面、把证据拿出来的人，必须还是主角或关键关系角色。',
    '师父、执事、长老不能带着新证据入场替主角揭底；他们最多只负责验真、压时限、截停。关键证据必须先由主角或情感杠杆角色碰到、藏到、换到、抢到或逼出来。',
    '情感杠杆角色不能只做人质或陪跑，至少要主动带出一份证据、一个条件、一次交易、一次反咬或一次站队变化。',
    '情感杠杆角色至少一次亲手保管、调包、递送、换出或毁掉一件实物筹码（账册、鳞片、钥匙碎片、血书、衣物、药瓶至少一种），不能只负责传话或哭喊。',
    '当前 5 集批次里，至少两集要由主角或情感杠杆角色亲自拿出证据、换条件、诈供、截人或反咬完成关键推进；师父、长老最多只负责认证、施压或给通道，不能做第一揭底人。',
    '公审、议事、长老会只准当压力容器，不能连续两集占主场；同一阶段里，公审/议事/殿内对质最多只允许 1 集当主战场，一旦用了，下一集就要转去潜入、追逃、交易、拦截、抢人、毁契或路上反打。',
    '后六集整段里，程序场主场总数最多 2 集，而且默认不能占 scene1；只有上游摘要明确写“当场对质/公审落锤”时，才允许它做本集的一场，而且必须是最短的一场。',
    '不要把后半程写成“议事殿对质 -> 廊道威胁 -> 夜潜取证 -> 再回殿里质询”的循环；如果上一集用了殿内场，这一集第一场就必须落在山林追逃、密室套话、旧巢取证、医庐伤势、抢人、埋伏或静室交易里。',
    '一旦宗门、官府或组织问责入场，它只负责压时限和改规则；真正戏眼要落在押送路上、门外堵截、住处搜物、路上投毒、暗巷换手、静室交易、山门截人、潭边封口这些动作上，不要把“被带去问话”本身写成整集。',
    '第4-7集如果上一集已经用了程序场，这一集第一场必须立刻转去路上、山林、医庐、旧屋、潭边、宅邸或暗巷，不能再让堂上场连续两集坐在 scene1。',
    '中段若上一集 scene1 已是程序场，这一集 scene1 必须改成路上、旧屋、山林、医庐、暗巷或门外动作；不要再从堂上场开场。',
    '同一集里，程序场最多只允许 1 场；如果还有下一拍，必须搬到门外、押送路上、住处、暗巷、山林或医庐继续推进。',
    '当前 5 集批次如果出现其他道观、使者、长老问责或更高层表态，他们只能拿现有旧账加压，不能替代黎明、小柔、李科、钥匙、伤势这条主线；summary 里不准把“外部问责/镇守重议/权责重排”写成第一戏眼。',
    '当前 5 集批次如果必须出现执事、外门执事、偏殿、公议或合议，它们只能做半句盖章：收证、定时限、押去潭边、转身离开。summary 和 sceneByScene 的真正推进要立刻切回石阶伏击、潭边抢纸、驿路截副本、医庐急醒、山门追人或旧巢夺账。',
    ...
    '小柔这类情感杠杆角色在中后段至少一次主动改局：传信、藏证、换条件、自救、反咬、拖时间，至少一种成立。',
    '当前 5 集批次的收口段主推进优先落在伤势处理、残党动作、证据换手、新职责落身；宗门追责、内部清洗、权位重排只能当后果背景，不能自己变成每集主戏眼。',
    ...
    '当前批次末两集如果必须出现接任、宣判、认罚、废修为或宗门表态，它们只能放在最后最短一场做结果确认；scene1 和主推进场必须落在带伤处理、截人、抢证、烧钥匙、护人或藏账。',
    '当前批次末集的余波优先落在人际站位、职责变化、证据外流、伤势代价和旧账未清，不要临时抬出更大怪物、更高封印层或新世界秘密来抢走本批次终点，也不要引入新名字、新亲属、新残党头子接管尾声。',
    '当前批次末集最多只允许 1 场制度确认；其余场次都要落在主角、小柔、残党、伤势代价或新职责的现实处理上。',
    '当前批次末集如果出现合议、接任、令牌或职责确认，它只能放在中段最短一场；尾场必须落在残党盯梢、见面约、证据外泄、水潭异动、伤势反噬或旧账追上门，不准用制度结果收尾。',
    '收束段的当前批次末集若保留制度确认场，它只能做最短第三场：1句起手 + 1句结果，不准把职责令牌、新看守职责或侧殿合议写成尾钩。',
    '当前批次末集第一场不准从宗门合议、卷轴宣读、长老落锤或制度宣判起手；必须从上一集留下的伤势、血契、碎钥匙、残党动作或未完人账直接开场。',
    '当前 5 集批次如果场景落在包扎、疗伤、潭边、锁旁、静室或歇脚处，不准把拉扯写成“为什么藏到现在/为什么不争/师父说过什么”的问答戏；要改成藏账页、压伤口、换路线、盯残党、塞钥匙碎片、决定先护谁。',
    '当前 5 集批次若主题必须显形，只能贴着空锁、碎钥匙、血迹、账页、盯梢人或已发生的代价落一句短狠结果，不准预埋“守空才能不争”“空的才是真的”这类定义句。',
