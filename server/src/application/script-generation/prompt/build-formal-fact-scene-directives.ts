import type { OutlineDraftDto, FormalFact } from '@shared/contracts/workflow'
import { isFormalFactSemanticLabel } from '@shared/domain/formal-fact/semantic-label'

function getConfirmedFact(outline: OutlineDraftDto, label: string): FormalFact | undefined {
...
export function buildFormalFactSceneDirectives(
  ...
  if (antagonistPressure) {
    ...
    lines.push(
      `首场戏里，${opponent}至少要说出一句带威胁的硬话，直接冲着主角要守的人或要守的东西去。`
    )
  }
  ...
  if (masterRole) {
    ...
    lines.push(
      `首场戏里，${master}不能只当背景设定，必须通过旧话、旧规矩、旧交代、旧物件中的至少一种，直接卡住主角当场的选择。`
    )
    lines.push(
      `首场戏里，观众必须看见主角不是单纯不敢动，而是被${master}留下的规矩或交代硬压着不动。`
    )
  }
  ...
  if (antagonistPressure && masterRole) {
    ...
    const master = pickName(masterRole.description)
    lines.push(
      `首场戏里，要让${opponent}的逼压和${master}留下的规矩正面撞上，形成"外面逼、心里也被旧规矩卡住"的双重压强。`
    )
  }
