...
// 验证是否有可用的 API Key
export function hasValidApiKey(config: RuntimeProviderConfig): boolean {
  return Boolean(
    (config.lanes.deepseek && config.deepseek.apiKey) ||
    (config.lanes.openrouterGeminiFlashLite && config.openrouterGeminiFlashLite.apiKey) ||
    (config.lanes.openrouterQwenFree && config.openrouterQwenFree.apiKey)
  )
}
