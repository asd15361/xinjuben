...
async function readProjectSnapshot(userDataDir) {
  ...
  return Object.values(data.projects || {}).find((project) => project.name === P0_REAL_REGRESSION_PROJECT_NAME) ?? null;
}
...
export async function runVisibleP0Regression() {
  ...
  try {
    ...
    const projectCard = page.getByRole('button').filter({ hasText: P0_REAL_REGRESSION_PROJECT_NAME }).first();
    await projectCard.waitFor({ state: 'visible', timeout: 30_000 });
    await projectCard.click();

    await page.waitForSelector(`text=/项目：\\s*${escapeRegExp(P0_REAL_REGRESSION_PROJECT_NAME)}/`, { timeout: 30_000 });
    await page.waitForTimeout(1000);

    await page.getByRole('button', { name: /剧本定稿/ }).click();
    await page.getByRole('heading', { name: '剧本定稿', exact: true }).waitFor({ state: 'visible', timeout: 30_000 });
    await page.getByRole('button', { name: /一键执笔生成|现在开始写剧本/ }).first().waitFor({ state: 'visible', timeout: 30_000 });
      ...
      await page.waitForFunction(
        () =>
          Array.from(document.querySelectorAll('button')).some(
            (button) => /(一键执笔生成|现在开始写剧本)/.test((button.textContent || '').trim()) && !button.disabled
          ),
        { timeout: 10_000 }
      ).catch(() => null);
    }
