...
function judgeQuality(project) {
  const outlineCount =
    project?.outlineDraft?.summaryEpisodes?.filter((item) => item?.summary?.trim()).length || 0
  ...
  const namedCharacters =
    project?.characterDrafts?.filter((item) => item?.name?.trim() && item?.goal?.trim()).length || 0
...
async function main() {
  ...
  try {
    ...
    await page.screenshot({
      path: path.join(outDir, 'real-outline-character-result.png'),
      fullPage: true
    })
