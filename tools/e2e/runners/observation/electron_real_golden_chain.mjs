...
async function main() {
  ...
  try {
    ...
    const outlineProject = await waitForProject(
      projectFile,
      (project) =>
        Boolean(project?.outlineDraft?.summaryEpisodes?.length && project?.characterDrafts?.length),
      240_000,
      'outline_ready'
    )
    ...
    await tryFillAct(
      0,
      `开局：${factAnchor}先压下来，反派恶霸拿小镇少女逼少年守钥人交出密库钥匙，上一场的威胁直接变成这一场的行动。`
    )
    await tryFillAct(
      1,
      `中段：反派恶霸继续加码，密库钥匙与山中妖物异动开始并线，少年守钥人每次反制都留下新的后果和新的误解。`
    )
    await tryFillAct(
      2,
      '高潮：小镇少女与密库钥匙背后的旧真相被掀开，反派恶霸把前面积累的债一次性压上来，逼少年守钥人正面亮底。'
    )
    await tryFillAct(
      3,
      '终局：少年守钥人把守约与救人的冲突一起推到台前，当前危机收口，但新的锁孔与新的代价继续承接到下一轮。'
    )
    await page.waitForTimeout(400)

    await page
      .getByRole('button', { name: /剧本定稿/ })
      .first()
      .click()
    await page.waitForTimeout(1000)
    await page
      .getByRole('heading', { name: /剧本定稿/ })
      .waitFor({ state: 'visible', timeout: 30_000 })
    const scriptProject = await readProjectSnapshot(projectFile)
    const scriptGenerateButton = page
      .getByRole('button', { name: /一键执笔生成|现在开始写剧本|启动真实生成 Gate/ })
      .first()
    ...
    const project = await waitForProject(
      projectFile,
      (currentProject) =>
        (currentProject?.scriptDraft?.length || 0) >= 1 && !currentProject?.generationStatus,
      300_000,
      'script_ready'
    )
    console.log(
