...
async function main() {
  ...
  try {
    ...
    await page.screenshot({ path: path.join(outDir, 'electron_after_create_project.png'), fullPage: true });
    ...
    const debug = await page.evaluate(() => {
      ...
      const projectButtons = Array.from(document.querySelectorAll('button'))
        ...
        .filter((t) => t.includes('E2E') || t.includes('Recent Projects') || t.includes('未选择项目'))
        .slice(0, 20);
    ...
    if (postGenerate.fail && !postGenerate.hasOutlineBtn) {
      ...
      if (postGenerate.fail && !postGenerate.hasOutlineBtn) {
        await page.screenshot({ path: path.join(outDir, 'electron_after_chat_generate_failed.png'), fullPage: true });
    ...
    await page.screenshot({ path: path.join(outDir, 'electron_after_chat_generate.png'), fullPage: true });
    ...
    await page.getByRole('heading', { name: '粗纲设计' }).waitFor({ timeout: 15000 }).catch(() => {});
    await page.waitForTimeout(600);
    await page.waitForSelector('text=婚约原件', { timeout: 15000 }).catch(() => {});
    await page.screenshot({ path: path.join(outDir, 'electron_outline_after_generate.png'), fullPage: true });
    ...
    await page.screenshot({ path: path.join(outDir, 'electron_formal_fact_confirmed.png'), fullPage: true });
    ...
    await page.screenshot({ path: path.join(outDir, 'electron_character_stage.png'), fullPage: true });
    ...
    await page.waitForTimeout(600);
    await page.screenshot({ path: path.join(outDir, 'electron_after_click_detailed_outline.png'), fullPage: true });
    const headings = await page.evaluate(() =>
      Array.from(document.querySelectorAll('h2')).map((h) => (h.textContent || '').trim()).filter(Boolean)
    );
    ...
    await page.waitForSelector('text=���ҳ�����õ� confirmed ��ʵ', { timeout: 15000 });
    await page.screenshot({ path: path.join(outDir, 'electron_detailed_outline_before_fill.png'), fullPage: true });
    ...
    await page.screenshot({ path: path.join(outDir, 'electron_script_before_gate.png'), fullPage: true });
    ...
    const gateProbe = await page.evaluate(() => {
      ...
      const planTitle = Array.from(document.querySelectorAll('p')).find((p) => (p.textContent || '').trim() === '执行计划');
    ...
    await page.screenshot({ path: path.join(outDir, 'electron_script_after_generation.png'), fullPage: true });
