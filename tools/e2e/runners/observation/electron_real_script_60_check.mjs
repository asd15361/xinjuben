      ...
      JSON.stringify(
        {
          ...
          boardWindow: result.generation?.board?.batchContext
            ...
            : null,
          issueCount:
            result.generation?.postflight?.issues?.length ||
            afterProject?.scriptStateLedger?.postflight?.issues?.length ||
            0,
          firstScene: afterProject?.scriptDraft?.[0] || null,
          postflightIssues:
            result.generation?.postflight?.issues ||
            afterProject?.scriptStateLedger?.postflight?.issues ||
            []
        },
        null,
        2
