...
async function runOnce(input) {
  ...
  try {
      ...
      try {
        ...
        if (Array.isArray(list) && list.some((item) => item?.name === input.projectName)) {
          ...
          await page.screenshot({
            path: path.join(outDir, `electron_${input.tag}_project_not_found.png`),
            fullPage: true
          })
    ...
    await page
      .waitForSelector(`text=/\\/\\s*(chat|outline|character|detailed outline|script)/`, {
        timeout: 30000
      })
      .catch(() => {})
    await page.waitForTimeout(1200)
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(outDir, `electron_${input.tag}_chat_disabled.png`),
        fullPage: true
      })
    ...
    if (chatResult.startsWith('生成失败')) {
      await page.screenshot({
        path: path.join(outDir, `electron_${input.tag}_generate_failed.png`),
        fullPage: true
      })
      ...
      if (chatResult2.startsWith('生成失败')) {
        ...
        if (chatResult2.includes('项目不存在') || chatResult2.includes('写入失败')) {
          ...
          if (chatResult3.startsWith('生成失败')) {
            await page.screenshot({
              path: path.join(outDir, `electron_${input.tag}_generate_failed_third.png`),
              fullPage: true
            })
          ...
          await page.screenshot({
            path: path.join(outDir, `electron_${input.tag}_generate_failed_retry.png`),
            fullPage: true
          })
    ...
    await page.screenshot({
      path: path.join(outDir, `electron_${input.tag}_after_confirm.png`),
      fullPage: true
    })
    ...
    await page.screenshot({
      path: path.join(
        outDir,
        input.tag === 'fail' ? 'electron_fail_before_gate.png' : 'electron_resume_before_gate.png'
      ),
      fullPage: true
    })
    ...
    if (gateProbe.gateDisabled) {
      await page.screenshot({
        path: path.join(outDir, `electron_${input.tag}_gate_disabled.png`),
        fullPage: true
      })
    ...
    if (input.tag === 'fail') {
      ...
      await page.waitForTimeout(1200)
      await page.screenshot({
        path: path.join(outDir, 'electron_resume_after.png'),
        fullPage: true
      })
    }
  } finally {
