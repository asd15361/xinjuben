...
export const CASE_ID = 'rw-b'
export const SEED_VERSION = 'sq-rw-b-stress'

// RW-B: Rewrite + Repair stress — all 10 episodes written with overflow/weak scenes
// Board shows completed but with overflow; failure resolution is 'failed' (not 'retry')
// This tests rewrite + repair chain: rewrite should收口 overflow, repair should not break structure

function buildOverflowScript() {
  // 10 valid episodes + 2 overflow weak episodes
  const validEpisodes = [
    {
      ep: 1,
      heading: '旧库前·日',
  ...
  // Overflow episodes — weak, short, no pressure/hook
  const overflowEpisodes = [
    {
      ep: 11,
      heading: '（溢出1）某处',
      action: '林守钥走了一段路。',
  ...
  const valid = validEpisodes.map(({ ep, heading, action, dialogue, emotion }) => ({
    sceneNo: ep,
    episodeNo: ep,
  ...
  const overflow = overflowEpisodes.map(({ ep, heading, action, dialogue, emotion }) => ({
    sceneNo: ep,
    episodeNo: ep,
    sceneHeading: heading,
    action,
...
function buildProgressBoard() {
  ...
  // Overflow episodes not tracked in board
  return {
    ...
    batchContext: {
      ...
      reason: 'All 10 episodes completed with overflow — rewrite+repair requested',
...
export function buildStore(projectOverrides = {}) {
  const projectId = `sq-rw-b-${Date.now().toString(36)}`
  const overflowScript = buildOverflowScript()
  const project = {
    id: projectId,
    name: `RW-B压力-${Date.now().toString(36)}`,
    ...
    scriptDraft: overflowScript,
    scriptProgressBoard: buildProgressBoard(),
    scriptFailureResolution: {
      kind: 'failed',
      reason:
        'Previous generation produced overflow weak scenes beyond target range. Repair+rewrite requested.',
      errorMessage: 'draft_coverage_insufficient: overflow detected, quality degraded',
      eventId: `evt_rw_b_fail_${Date.now().toString(36)}`,
      lockRecoveryAttempted: false
    },
    scriptRuntimeFailureHistory: [
      {
        episodeNo: 11,
        code: 'draft_coverage_insufficient',
        timestamp: FIXTURE_TIMESTAMP,
        detail: 'Episode 11 generated with insufficient coverage and quality'
      },
      {
        episodeNo: 12,
        code: 'draft_coverage_insufficient',
        timestamp: FIXTURE_TIMESTAMP,
        detail: 'Episode 12 overflow scene is weak with no hook or pressure'
      }
    ],
