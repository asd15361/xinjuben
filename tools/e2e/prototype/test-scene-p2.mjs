...
const qualityPath =
  'file://' + path.resolve(REPO_ROOT, 'src', 'shared', 'domain', 'script', 'screenplay-quality.ts')
  ...
  },
  {
    ...
    scenes: [
      ...
      },
      {
        ...
        tension:
          '师父的禁令被打破。林守钥第一次动武，打手已倒地，血已经流出来了——代价以最原始的方式开始兑现。',
        hookEnd:
          '林守钥将最后一个打手打倒在地，嘴角渗血的打手甲捂着肋部瘫在墙角，代价已经以血的形式落在了地上。',
        budgetChars: 350,
        prevSceneOutcome:
          '林守钥深夜发现警告字条，三日期限已下，将弟弟送到舅舅家，第一次感到真正代价。',
  ...
  },
  {
    ...
    scenes: [
      ...
      },
      {
        ...
        tension:
          '沈黑虎知道有暗格但没找到东西，铜钥匙已被转移到窗棂缝隙里。库房外，敌人的火把光已经照进来了。',
  ...
  },
  {
    ...
    scenes: [
      ...
      },
      {
        ...
        budgetChars: 350,
        prevSceneOutcome:
          '林守钥找到山中猎户小屋躲避，旧友沈云告知有秘道通往县衙旧档库，终于找到翻盘的路。',
...
for (const episode of EPISODE_DATA) {
  ...
  for (const scene of episode.scenes) {
    ...
    try {
      ...
      console.log(
        `  [${scene.sceneCode}] chars=${chars}/${scene.budgetChars} ${chars > scene.budgetChars ? '⚠️ OVER' : '✅'}`
      )
      ...
      fs.writeFileSync(
        path.join(outDir, `ep${episode.episodeNo}-${scene.sceneCode}-raw.txt`),
        raw,
        'utf8'
      )
    } catch (err) {
  ...
  if (successful.length !== episode.scenes.length) {
    console.log(
      `  Assembly SKIPPED — not all scenes succeeded (${successful.length}/${episode.scenes.length})`
    )
    allResults.push({
      episodeNo: episode.episodeNo,
      sceneResults,
  ...
  if (parsed) {
    try {
      ...
      console.log(
        `  quality: problems=${quality.problems.length === 0 ? 'none' : quality.problems.join(', ')}`
      )
