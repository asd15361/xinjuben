...
- **主视图也是验收面，不只是复制文本**：用户会直接看/手动复制人物卡片。五维字段如果只在复制按钮里出现，用户仍会判断“小传缺外在形象、性格、身份、价值观、剧情作用”。
- **“一碰就炸”会误导男频主角行为**：`conflictTrigger` 作为内部字段没问题，但用户可见要叫“被逼动作点”，并要求主角以隐忍、证据、后手、底牌或规则反咬行动，不能写成炸毛崩溃。
- **多阵营和低成本不冲突**：用户要的是角色/阵营底账丰富，不是拍摄场景无限扩张。短剧应让 3-5 个高复用场景承载多阵营冲突。
- **角色丰富不等于演员无限多**：阵营底账要复杂，但人物池要按集数控量；20 集优先少核心、多复用，60 集再扩。功能位要用 `reusableRoleKey/reuseSceneKeys` 变成可调度资产。
- **爽点要绑定到人，不只绑定到集**：核心人物如果没有 `payoffTags`，后续骨架只知道他“重要”，不知道他负责身份碾压、证据打脸、隐藏大佬撑腰还是反派自食其果。
- **爽点字段要标准化，不要把短名当结构真相**：`规则反噬/挑衅反杀/底牌隐藏` 用户能看懂，但结构字段必须落成 16 种标准名，否则骨架、详纲、控制卡、剧本片段会看似一致但整体口径漂移。
- **端到端链路要验正文，不只验 DTO**：payoff 字段四层一致仍不够，正式剧本正文必须出现 `payoffOwnerName/payoffTargetName/payoffScene` 并演出 `payoffExecution`，否则只是账本对齐，戏没落地。
