...
function collectHardAnchors(outline: OutlineDraftDto, script: ScriptSegmentDto[]): string[] {
  const latestScenes = script.slice(-2).map((scene) => `${scene.action} ${scene.dialogue} ${scene.emotion}`).join('\n')
...
export function buildStoryMomentum(input: {
  ...
  return {
    ...
    activeConflictLine: latestScene?.action || input.outline.mainConflict,
    pendingCost: latestScene?.emotion || '上一场动作带来的代价尚待兑现',
    memoryEchoes,
    hardAnchors
  }
}
