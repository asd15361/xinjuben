...
function buildScriptMarkdown(project: ProjectSnapshotDto): string {
  ...
  const overflowEpisodes = collectOverflowScriptEpisodeNos(project.scriptDraft ?? [], targetEpisodes)
  const orderedScript = [...(project.scriptDraft ?? [])].sort((left, right) => left.sceneNo - right.sceneNo)
  ...
  return [
    ...
    ...(overflowEpisodes.length > 0 ? [`- 超出计划集数：第 ${overflowEpisodes.join('、')} 集`] : []),
    '',
