/**
 * validate-stage-input-contract.ts — Shared stage input validation
 *
 * This is the SINGLE SOURCE for validateStageInputContract logic.
 * Both server and main use this shared version — no duplication.
 *
 * PRINCIPLES:
 * - Server is the sole authority for formal state decisions
 * - Main process uses this for local validation only (no formal state writes)
 * - Renderer never computes validation locally — always via HTTP or IPC
 */

import type { InputContractValidationDto, InputContractIssueDto } from '../../contracts/input-contract.ts'
...
export function validateStageInputContract(
  ...
  const issues: InputContractIssueDto[] = []
