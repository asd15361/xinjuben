...
export function detectDramaProgressionDimensions(text: string): DramaProgressionDimensionsDto {
  return {
    ...
    opposition:
      /(阻碍|压制|威胁|追杀|围堵|打压|陷害|逼迫|拦截|对抗|搜身|围了上来|踢翻|抓走|带走|施压)/i.test(
        text
      ),
    cost: /(代价|失去|牺牲|受伤|暴露|损耗|反噬|付出|护不住|失望|恐惧|窝囊|被迫|发烫|午时前)/i.test(
      text
    ),
    relationship:
      /(信任|背叛|和解|站队|联手|反目|情感|关系|所爱|心上人|小柔|师父|守护|护不住人|眼神复杂|失望)/i.test(
        text
      ),
    hook: /(悬念|钩子|伏笔|秘密|证据|倒计时|未解|反转|下一步|搜身|带走|苏醒|闷响|骑马而来|午时前|要么)/i.test(
      text
    )
  }
}
