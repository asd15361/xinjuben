...
export function ValidationBadge({ score, label }: BadgeProps) {
    const color =
    ...
    return (
        <span className={`inline-flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1 rounded-full border ${color}`}>
...
export function WorkspaceInput({ label, placeholder, value, onChange, hint, multiline, rows = 4, disabled }: InputProps) {
    return (
...
export function HintPanel({ hints, title = 'AI 赋能建议' }: HintPanelProps) {
        ...
        <div className="space-y-3">
            ...
            <AnimatePresence>
                {hints.map((hint, i) => {
                    ...
                    const borderColor =
                        hint.confidence === 'high' ? 'border-orange-500/30'
                            : hint.confidence === 'mid' ? 'border-yellow-500/20'
                                : 'border-white/10';
                    const dotColor =
                        hint.confidence === 'high' ? 'bg-orange-500'
                            : hint.confidence === 'mid' ? 'bg-yellow-400'
                                : 'bg-white/20';
                        ...
                        >
                            ...
                            <div className="flex items-center gap-1.5 mt-3">
                                ...
                                <span className="text-[9px] uppercase tracking-widest text-white/20">
                                    {hint.confidence === 'high' ? '高置信度' : hint.confidence === 'mid' ? '中置信度' : '低置信度'}
                                </span>
                            </div>
                        </motion.div>
                    );
