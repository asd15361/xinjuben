...
export function OutlineEntityStorePanel(input: { entityStore: ProjectEntityStoreDto | null }) {
    ...
    <div className="rounded-2xl border border-white/8 bg-white/[0.02] p-5 space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-1">
          <p className="text-[10px] uppercase tracking-widest text-white/25 font-bold">世界底账观察口</p>
          <p className="text-sm font-black text-white/85">先看系统已经识别出哪些势力、人物和关系。</p>
      ...
      {viewModel.isEmpty ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
          <div className="rounded-xl border border-white/8 bg-black/15 p-4 space-y-3">
            <p className="text-[11px] font-black text-white/70 uppercase tracking-widest">已识别势力</p>
            {viewModel.factions.length > 0 ? (
              viewModel.factions.map((item) => (
                <div key={item.name} className="space-y-1">
                  ...
                  <p className="text-[11px] text-white/45 leading-relaxed">{item.summary || '暂无摘要'}</p>
                </div>
              ))
            ) : (
          ...
          <div className="rounded-xl border border-white/8 bg-black/15 p-4 space-y-3">
            <p className="text-[11px] font-black text-white/70 uppercase tracking-widest">已识别人物</p>
          ...
          <div className="rounded-xl border border-white/8 bg-black/15 p-4 space-y-3">
            <p className="text-[11px] font-black text-white/70 uppercase tracking-widest">已识别关系</p>
