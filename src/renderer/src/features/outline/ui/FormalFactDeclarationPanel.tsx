...
export function FormalFactDeclarationPanel(input: {
  ...
  const validation = useMemo(() => {
    ...
    return FactEngine.validateCoreFact(draft.description, input.outline.mainConflict, input.outline.theme)
  ...
  return (
    <div className="bg-white/3 border border-white/8 rounded-2xl p-6 space-y-4">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-widest text-white/25">核心设定确认</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <label className="block space-y-2">
          <span className="block text-[10px] uppercase tracking-widest text-white/25 font-bold">设定级别</span>
          <select
            value={draft.level}
      ...
      {validation && validation.suggestions.length > 0 && (
        <div className="bg-yellow-500/5 border border-yellow-500/15 rounded-xl p-4 space-y-2">
          <p className="text-[10px] font-bold uppercase tracking-widest text-yellow-400/60">这一条还可以再补强</p>
