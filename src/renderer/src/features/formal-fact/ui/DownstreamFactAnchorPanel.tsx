...
export function DownstreamFactAnchorPanel(props: DownstreamFactAnchorPanelProps): JSX.Element {
  ...
  return (
    <section className="rounded-[28px] border border-white/10 bg-[linear-gradient(135deg,rgba(255,255,255,0.06),rgba(16,16,16,0.32))] px-5 py-5 space-y-4">
      <div className="space-y-2">
        <p className="text-[10px] uppercase tracking-[0.32em] text-white/25 font-bold">
          已确认的核心设定
        </p>
      ...
      <div className="grid gap-3 2xl:grid-cols-[minmax(0,1.4fr)_minmax(260px,0.9fr)]">
          ...
          {locked ? (
            ...
            <div className="space-y-2">
              {facts.map((fact) => (
                <div
                  key={fact.id}
                  className="rounded-xl border border-emerald-400/15 bg-emerald-400/6 px-4 py-3"
                >
                  <div className="flex items-center justify-between gap-3">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/4 px-4 py-4 space-y-3">
          ...
          <div
            className={`rounded-xl border px-3 py-3 ${locked ? 'border-yellow-400/20 bg-yellow-500/8' : 'border-emerald-400/15 bg-emerald-500/8'}`}
          >
            <p
              className={`text-[10px] uppercase tracking-[0.22em] font-bold ${locked ? 'text-yellow-200/70' : 'text-emerald-300/75'}`}
            >
              {locked ? '先回粗纲补这一步' : '可以继续往下写'}
            </p>
            <p
              className={`mt-1 text-[11px] leading-relaxed ${locked ? 'text-yellow-100/72' : 'text-white/60'}`}
            >
              {locked
