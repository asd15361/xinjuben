...
- Gemini 在旧项目里不是单模型，而是明确分成 `flash` 和 `pro` 两条 lane
...
- 默认 DeepSeek
- 长上下文 / 跨工序 / 详纲 / 人物小传走 Gemini Flash
- P0 风险 / 结局审校 / 全季审校走 Gemini Pro
...
- [gemini-provider.ts](/D:/project/juben/src/generation/pipeline/gemini-provider.ts)
...
- `MODEL_ROUTER_ENABLE_GEMINI_FLASH`
- `MODEL_ROUTER_ENABLE_GEMINI_PRO`
- `RUNTIME_FETCH_TIMEOUT_MS`
